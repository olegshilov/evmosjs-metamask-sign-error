...
function createCosmosService(
  ...
  async function broadcastTransaction(txToBroadcast: TxToSend) {
    console.log("broadcastTransaction", { txToBroadcast });
    try {
      const { data: broadcastResponse } = await axios.post<{
      ...
      if (broadcastResponse.tx_response.code !== 0) {
        throw new Error(
          `Transaction Failed: ${broadcastResponse.tx_response.raw_log}`
        );
      }

      return broadcastResponse.tx_response;
    } catch (error) {
      throw new Error((error as any).message);
    }
  }
  ...
  async function simulateTransaction(
    ...
    console.log("simulateTransaction", { txToBroadcast });
    try {
